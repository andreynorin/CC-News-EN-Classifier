...
# splitting the dataset int 100k and 10k records for train/test
trainDataset = tempDF1.sort('index').limit(100000)
testDataset = tempDF1.sort('index', ascending = False).limit(10000)
...
testDataset_preds_df.write.format('parquet').option('header','true').save("s3a://litter-box/fromHDFS/sample-dataset-predictions.parquet", mode='overwrite')
...
preds_df.write.format('parquet').option('header','true').save("s3a://litter-box/fromHDFS/sample-cc-news-en-clean-predictions.parquet", mode='overwrite')